...
本教程带你在 Nexus-AI 平台上从零构建一个 **技术博客生成 Agent**：给它一个技术主题和几项偏好参数，它会自动检索公开技术资料、分析架构与原理、总结最佳实践，最终生成一篇结构完整的 Markdown 技术博文并保存到本地。

## 你会得到什么

- 一个可单轮或交互式调用的 **技术写作 Agent**，输入一句话需求即可产出 1000~4000 字的 Markdown 博文。
...
2. 点击「新建项目」，在对话框中填写：
   - **项目名称**：`tech-blog-generator`
   - **架构类型**：`Single Agent`
   - **描述**：`面向任意技术主题的 Markdown 博文生成助手`
3. 点击「创建」，系统会为你分配一个 `Project ID`。
...
```text
我需要一个技术博客生成 Agent：
- 通过 http_request 抓取目标主题的公开技术资料（GitHub、官方文档等）
...
切换到「代码」标签页，点击「生成代码」。平台会产出一份名为 `tech_blog_generator.py` 的 Python 脚本。打开预览，确认包含以下结构：
...
```bash
python tech_blog_generator.py -i "帮我写一篇面向初级开发者的 LangChain 入门文章"
```
...
[阶段 1/5] 正在检索 LangChain 相关技术资料...
[阶段 2/5] 正在分析 LangChain 系统架构...
[阶段 3/5] 正在解析 LangChain 工作原理...
[阶段 4/5] 正在总结应用最佳实践...
[阶段 5/5] 正在生成技术博文...
[生成摘要] 字数：约 2100 字 | 配置：初级开发者/入门/standard | 信息来源：在线 | 耗时：约 42 秒
```

打开 `.cache/&lt;agent_id&gt;/&lt;session_id&gt;/` 目录，应能看到 `tech_blog_&lt;timestamp&gt;.md` 文件。

<!-- SCREENSHOT: local-run -->

::: warning
若 `[阶段 1/5]` 显示「切换离线模式」，说明当前网络无法访问目标站点，Agent 会回落到基于预训练知识生成博文，并在末尾免责声明中标注「信息来源：离线」。
:::
...
```json
{
  "prompt": "生成一篇深度解析 Kubernetes 调度器实现原理的技术博文，面向中高级开发者，字数 3000 以上"
}
```
...
- 流式响应依次输出五个阶段的进度标记。
- 最终摘要字段显示字数 ≥ 3000。
- `.cache/.../tech_blog_*.md` 文件已生成，包含：标题、摘要、目录、引言、架构解析、工作原理、最佳实践、总结、信息来源、免责声明 共 10 个章节。
...
- 阅读 [构建 Hermes 分析 Agent](./build-hermes-analyst.md) 查看面向单一框架的同类教程。
- 阅读 [Agent 构建小贴士](./tips.md) 了解提示词调优与工具集编排的实战经验。
- 阅读 [Agent 工作台](../using/create-agent.md) 了解 Build Workflow V2 的完整生命周期。
